document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.location.href.includes('/death/edit/')) { 
        $(document).ready(function () {
            // Extract the death ID from the URL
            const urlParts = window.location.pathname.split('/');
            const deathID = urlParts[urlParts.length - 1]; // Get the last part of the URL

            if (deathID) {
                fetchDeathDetails(deathID);
            }
        });
        
        function fetchDeathDetails(id) {
            $.ajax({
                url: `/death/${id}`,
                type: 'GET',
        ...
        function fillEditForm(rowData) {
            ...
            $('#editButtonDeath').attr('data-id', rowData.id);
            ...
            $('#editdeath_date').val(formatDate(rowData.death_date));
            $('#editburial_date').val(formatDate(rowData.burial_date));
            $('#editburial_place').val(rowData.burial_place);
            $('#editcause').val(rowData.cause);
            $('#editCP').val(rowData.contact_person);
            $('#editCP_address').val(rowData.cp_address);
    ...
    document.body.addEventListener('click', function(event) {
        if (event.target.matches('button[data-page="insertDeath"]')) {
            event.preventDefault(); // Prevent the default form submission
            const form = document.querySelector('#insertDeathForm');
            const formData = new FormData(form);
            const data = Object.fromEntries(formData.entries());
            console.log(data);
            console.log('BUTTON WAS PRESSED');
            insertDeath(data);
        } else if (event.target.matches('button[data-page="editDeath"]')) {
            event.preventDefault();
            console.log('Edit Death Pressed')
            const id = event.target.getAttribute('data-id');
            const form = document.getElementById('updateDeathForm');
            const formData = new FormData(form);
            const data = Object.fromEntries(formData.entries());
            data['death_id'] = id; 
            console.log(data);
            editDeath(data);
        } else if (event.target.matches('button[data-page="deleteDeath"]')) {
...
async function insertDeath(data) {
    try {
        const response = await fetch('/death/insert', {
            method: 'POST',
            headers: {
...
async function editDeath(data) {
    alert('edit death pressed')
    try {
        const response = await fetch('/death/update', {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function deleteDeath(id) {
    try {
        ...
        const response = await fetch(`/death/delete/${id}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            }
